...
void main() {
  ...
  group('短帧编码测试', () {
    test('基础短帧编码 - 默认启用校验和', () {
      ...
      // 验证命令
      expect(encoded[2], InterChipCmds.normal.value);
  ...
  group('长帧编码测试', () {
    test('基础长帧编码', () {
      ...
      // 验证命令
      expect(encoded[3], InterChipCmds.dfu.value);
      ...
      expect(encoded.length,
          305); // Flag + Len + LenH + Cmd + Payload(300) + Checksum
    });
  ...
  group('错误处理测试', () {
    test('负载过大抛出异常', () {
      ...
      expect(() => encoder.encode(packet), throwsA(isA<RangeError>()));
    });
    ...
    test('长度不匹配抛出异常', () {
      ...
      expect(() => encoder.encode(packet), throwsA(isA<ArgumentError>()));
    });
  });
