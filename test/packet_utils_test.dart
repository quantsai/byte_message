...
void main() {
    ...
    test('字节序转换往返测试', () {
      ...
      for (final value in testValues) {
        ...
        expect(reconstructed, value,
            reason: '值 0x${value.toRadixString(16)} 往返转换失败');
      }
    });
  });
  ...
  group('校验和计算工具测试', () {
    ...
    test('calculateXorChecksum - 复杂数据', () {
      expect(
          PacketUtils.calculateXorChecksum(
              [0x10, 0x04, 0xF8, 0x01, 0x02, 0x03]),
          0xEC);
    ...
    test('calculateXorChecksum - 无效输入抛出异常', () {
      ...
      expect(() => PacketUtils.calculateXorChecksum([0, 255, 256]),
          throwsArgumentError);
    });
    ...
    test('verifyXorChecksum - 无效校验和抛出异常', () {
      ...
      expect(
          () => PacketUtils.verifyXorChecksum(data, -1), throwsArgumentError);
      expect(
          () => PacketUtils.verifyXorChecksum(data, 256), throwsArgumentError);
    });
  });
  ...
  group('数据验证工具测试', () {
    test('isValidPayloadLength - 有效长度', () {
      ...
      expect(
          PacketUtils.isValidPayloadLength(PacketConstants.MAX_PAYLOAD_SHORT),
          true);
    ...
    test('isCommandProtocol - 命令协议检测', () {
      expect(
          PacketUtils.isCommandProtocol(PacketConstants.CMD_CONTROL_BUS), true);
      expect(
          PacketUtils.isCommandProtocol(0xF8), true); // CMD_CONTROL_BUS = 0xF8
    ...
    test('isValidPayload - 负载有效性验证', () {
      ...
      expect(PacketUtils.isValidPayload(List.generate(1000, (i) => i % 256)),
          true);
  ...
  group('格式化工具测试', () {
    ...
    test('formatBytes - 自定义分隔符', () {
      expect(
          PacketUtils.formatBytes([0x01, 0x02, 0x03], '-'), '0x01-0x02-0x03');
      expect(PacketUtils.formatBytes([0x01, 0x02, 0x03], ''), '0x010x020x03');
      expect(PacketUtils.formatBytes([0x01, 0x02, 0x03], ', '),
          '0x01, 0x02, 0x03');
    });
    ...
    test('parseHexString - 复杂格式', () {
      expect(
          PacketUtils.parseHexString('0x10, 0x04, 0xF8'), [0x10, 0x04, 0xF8]);
