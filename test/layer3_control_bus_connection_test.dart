...
void main() {
  ...
  group('Layer3 设备连接 - 请求连接协议', () {
    test('请求编码 - 协议版本 0x02（默认）', () {
      final payload = GetDeviceConnectionReq().encode();
      expect(payload, [0x02]);
    });

    test('请求编码 - 非法版本抛出 RangeError', () {
      expect(() => GetDeviceConnectionReq(protocolVersion: 256),
          throwsA(isA<RangeError>()));
      expect(() => GetDeviceConnectionReq(protocolVersion: -1),
          throwsA(isA<RangeError>()));
    });
    ...
    test('应答解码 - 全零示例（长度28）', () {
      ...
      final resp = GetDeviceConnectionRes.fromBytes(bytes);
      expect(resp.model, '');
    ...
    test('应答解码 - 长度不匹配抛出 ArgumentError', () {
      ...
      expect(() => GetDeviceConnectionRes.fromBytes(bad),
