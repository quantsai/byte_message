...
class DecodeResult {
  ...
  /// 创建失败的解码结果
  factory DecodeResult.failure(String error,
      {String? errorCode, int consumedBytes = 0}) {
    return DecodeResult(
      success: false,
      error: error,
      errorCode: errorCode,
...
class DecoderConfig {
  ...
  /// 默认的最大包长（稍大于最大可能的数据包长度）
  static const int defaultMaxPacketLength =
      PacketConstants.MAX_PAYLOAD_LONG + 1000;

  /// 默认配置实例，便于直接使用
  static const DecoderConfig defaults = DecoderConfig();
  ...
  const DecoderConfig({
    ...
    this.maxPacketLength = defaultMaxPacketLength,
  ...
  ///
  /// 参数说明：
  /// - strictChecksumValidation：是否严格校验校验和（为空时沿用当前值）
  /// - allowUnknownFlags：是否允许未知的标志位（为空时沿用当前值）
  /// - strictLengthValidation：是否严格验证长度（为空时沿用当前值）
  /// - maxPacketLength：最大允许的数据包长度（为空时沿用当前值）
  /// - debugMode：是否启用调试模式（为空时沿用当前值）
  ///
  /// 返回值：
  /// - 新的 DecoderConfig 实例，包含覆盖后的配置项
  DecoderConfig copyWith({
    ...
    return DecoderConfig(
      strictChecksumValidation:
          strictChecksumValidation ?? this.strictChecksumValidation,
      allowUnknownFlags: allowUnknownFlags ?? this.allowUnknownFlags,
      strictLengthValidation:
          strictLengthValidation ?? this.strictLengthValidation,
      maxPacketLength: maxPacketLength ?? this.maxPacketLength,
      debugMode: debugMode ?? this.debugMode,
    );
  }

  /// 以可读字符串形式展示当前配置（仅用于调试/日志）
  ///
  /// 返回值：
  /// - 包含所有配置项的字符串
  @override
  String toString() =>
      'DecoderConfig{strictChecksumValidation: $strictChecksumValidation, '
